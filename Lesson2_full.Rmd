...
x <- B %*% z + mu
print(colMeans(t(x)))
print(var(t(x))) ## var matrice ? comment est calculé ?
```
...
n <- 100
set.seed(222)
A <- rnorm(1) # une seule rea pour A
set.seed(223)
phi <- 2*pi*runif(1) # une seule rea pour phi 
lambda <- pi/6 # le nu
set.seed(224)
x <- A*cos(seq(1,n)*lambda + phi) # seq(1,n) le temps
plot(ts(x),xlab="Time",ylab="")
```
...
for(i in 1:100) { #pas de i dans la boucle

z <- rnorm(10)
x <- rep(0,10)
phi <- .9
x0 <- rnorm(1)/sqrt(1-phi^2) # init pour variance identique 
x[1] <- phi*x0 + z[1]
