...
- positive definite sequence exo 2
...
- revisiter chapitres cache te txt book à la lumiere de ça

### Concepts 

Partie 1: spectral density à partir des caracs du modele
- filters : square gaine function applied to input spectral density 
- prcatical computing and plotting 

2 elaborated examples 
- business cycle 
- HP filter 

Partie 2 (key!)

- def of fourier frequencies (interval)
vu la def: cas impait ne touche excatement ni -pi ni pi
cas pair: ne touche pas -pi, mais exactement pi


- spectral decomp of Toepliz matrices (asymp: same eingen vectors, diff in eingenv)
pour les valeurs de Q: valeurs des lambda avec k de 1 à n (en gros de -pi à pi: ok)

- important comparison
consider cov matrix (toepliz): direct extract eigenvalues (linalg)
compare to values of spectral density evaluated at fourier freq (depend de n sample size)
asymptotic conv 
- pourquoi idem pour inverse
...
Partie 4 
- model ident with acf pacf iacf

### R skills

- direct input of Toeplitz matrix
- solve eingen values, vectors

- plot spectral for MA(1) 
- for MA(q) given the autocorrelations (recursion= sum)
- spectral density for arma knowing coefficients of polynomials theta and phi, avec formule filtre

à voir (in better conditions) !!
- hp filter for seasonal data: business cycle extraction

- compute pacf ma(q), arma(p,q)


distinction : what can be done 

- from model 

ex MA: direct formula 
